...
func AddCountryRegion(region Region) (string, error) {
	...
	_, resultErr := stmt.Exec(region.ID, region.Name, region.CountryId)
	if resultErr != nil {
		return "", resultErr
	}
	...
	return region.Name + " successfully added to country regions", nil
}
...
func FetchCountryRegions(country string) []Region {
	...
	for rows.Next() {
		region := Region{}
		err = rows.Scan(&region.ID, &region.Name, &region.CountryId, &region.CreatedAt, &region.UpdatedAt)
		if err != nil {
			log.Fatal(err)
		}
